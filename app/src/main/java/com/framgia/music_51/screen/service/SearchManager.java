...
public class SearchManager implements MediaPlayer.OnCompletionListener,
    ...
    private List<Search> mTracks;
    private List<Search> mUnShuffleTracks;
    private Context mContext;
    private OnUpdateUISearchListener mListener;
    private String mUrl;
    private int mPosition;
    private PlayMode mPlayMode;

    public SearchManager(Context context, ArrayList<Search> tracks, int position) {
        mContext = context;
        mTracks = tracks;
        mPosition = position;
    ...
    public void setData(List<Search> tracks) {
        try {
    ...
    private void shuffle() {
        Search track = mTracks.get(mPosition);
    ...
    private void shuffleList(List<Search> tracks, Random random) {
        @SuppressWarnings("unchecked") final List<Search> trackSwap = tracks;
        if (tracks instanceof RandomAccess) {
            ...
            Search[] array = (Search[]) trackSwap.toArray();
            ...
            int i = 0;
            ListIterator<Search> it = trackSwap.listIterator();
            while (it.hasNext()) {
                it.next();
                it.set(array[i++]);
            }
